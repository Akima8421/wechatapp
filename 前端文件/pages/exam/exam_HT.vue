<template>
	<view>
		...
		<u-button @click="Eadd" size="default" type="warning" class="specialbtn2">发布考试+</u-button></view>
				...
				<view slot="body">
					...
					<view class="homework2">
						<u-row gutter="16">
							...
							<u-col span="4">
								<view>
									<u-button @click="E_goto(index)" :disabled="item.unok">成绩</u-button>
									<u-button @click="E_delete(index)">删除</u-button>
									</view>
							</u-col>
							<u-col span="4">
	...
	export default{
		...
		data(){
			return{
				...
				value2: 0,
				value3: 0,
				choosecid: uni.getStorageSync('mycid'),
				examlist:[],
			}
		},
		methods:{
			E_goto:function(index){
				uni.navigateTo({
					url:'./Echange?index='+ this.examlist[index].id+'&cid='+uni.getStorageSync('mycid')
				})
			},
			...
			E_flash:function(str){
				...
				uni.request({
					...
					data:{
						...
						sql: "select * from einfo where eIs = 'true' and eYear = '" + this.f_Year + "' and eTerm = '" + this.f_Term + "' and ecId = '" + this.choosecid + "'" +str
					},
					success: (res) => {
